...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]] = None) -> _TRAIN_CLS:
    ...
    # Check arguments
    # Stages that support predict_with_generate (text generation tasks)
    text_generation_stages = ["sft", "sft-meme_multiclass"]
    if finetuning_args.stage not in text_generation_stages:
        if training_args.predict_with_generate:
            raise ValueError(f"`predict_with_generate` cannot be set as True except for text generation stages: {text_generation_stages}.")
    ...
    if finetuning_args.stage in text_generation_stages and training_args.do_predict and not training_args.predict_with_generate:
