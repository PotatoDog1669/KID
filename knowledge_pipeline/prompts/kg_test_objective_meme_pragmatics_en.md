...
This prompt is used to construct test or inference data. The goal is to generate a general meme semantic-pragmatic description that can transfer across meme datasets and tasks, not a hidden explanation for one dataset's label set.

Do not guess the final class. Do not make the description more label-oriented than the image/text supports. Preserve the meme's factual and conventional meaning as accurately as possible.
