...
class KIDClassifierArguments:
    ...
    classifier_class_names: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Comma-separated class names for multiclass classifier prediction CSV columns. "
                "For example: individual,organization,community,society."
            )
        },
    )
    ...
    loss_ratio: Optional[List[float]] = field(default_factory=lambda: [1.0, 1.0])

    evaluate_only: bool = field(default=False)
    
    # Extraction loss parameters (for sft-meme_multiclass)
    # Enable extraction loss that computes classification loss from generated text
    enable_extraction_loss: bool = field(
        default=False,
        metadata={"help": "Whether to enable extraction loss that computes classification loss from generated text."}
    )
    extraction_loss_warmup_steps: int = field(
        default=0,
        metadata={"help": "Number of steps before extraction loss starts."}
    )
    extraction_loss_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for extraction loss."}
    )
...
class FinetuningArguments(
    SwanLabArguments, BAdamArgument, ApolloArguments, GaloreArguments, RLHFArguments, LoraArguments, FreezeArguments, KIDClassifierArguments
):
    ...
    classifier_checkpoint: Optional[str] = field(
        default=None, metadata={"help": "Path to the classifier checkpoint (e.g., classifier.bin)."}
    )
    load_classifier_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to directory containing classifier_config.json and classifier.bin for second-stage training."}
    )
