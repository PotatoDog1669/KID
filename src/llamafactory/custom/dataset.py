...
def get_values_from_gt(dataset, split):
    ...
    elif dataset == 'Memotion':
            # Humour, Sarcasm, Offense, Motivation (in this order)
            if split == "train":
                gt_file = "./data/gt/" + "Memotion" + "/" + "labels.csv"
                gt_df = pd.read_csv(gt_file)
                list_image = gt_df["image_name"].to_list()
                ...
                list_label = []
                # Get all four labels for each sample
                humour = gt_df["humour"].to_list()
                sarcasm = gt_df["sarcasm"].to_list()
                offensive = gt_df["offensive"].to_list()
                motivation = gt_df["motivational"].to_list()
                
                for h, s, o, m in zip(humour, sarcasm, offensive, motivation):
                    # Initialize multi-label vector [H, S, O, M]
                    label_vec = [0, 0, 0, 0]
                    ...
                    # Motivational
                    if m != "not_motivational":
                        label_vec[3] = 1
                    
                    list_label.append(label_vec)
                ...
                list_label = []
                labels_pool = gt_df_labels["Labels"].to_list()
                for label_str in labels_pool:
                    # Label format is like "label_0101"
                    label_digits = label_str.split("_")[1]  # Get the "0101" part
                    label_vec = [
                        int(label_digits[0]),  # Humour
                        int(label_digits[1]),  # Sarcasm
                        int(label_digits[2]),  # Offensive
                        int(label_digits[3])   # Motivational
                    ]
                    list_label.append(label_vec)
